...
function readBackendBaseUrl(): string {
  ...
  const configuredBackendBaseUrl = applicationExtraConfiguration?.backendBaseUrl;

  // Diese Zeile prüft, ob eine gültige Backend-Adresse vorhanden ist.
  if (typeof configuredBackendBaseUrl !== 'string' || configuredBackendBaseUrl.length === 0) {
...
function createImageFileName(imageUri: string): string {
  // Diese Zeile zerlegt die URI in einzelne Pfadteile.
  const uriPathParts = imageUri.split('/');

  // Diese Zeile liest den letzten Pfadteil als Dateinamen aus.
...
export async function analyzeImageWithBackend(imageUri: string): Promise<DetectionApiResponse> {
  ...
  if (!response.ok) {
    // Diese Zeile liest den Antworttext für die Fehlersuche.
    const errorText = await response.text();

    // Diese Zeile wirft einen Fehler mit zusätzlichem Status-Text.
    throw new Error(`Detection request failed with status ${response.status}: ${errorText}`);
  }
